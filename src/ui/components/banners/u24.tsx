...
const United24Banner: React.FC<United24BannerProps> = ({ animateGradient = true, onClose, showCloseButton = true }) => {
  ...
  const textColorClasses = animateGradient ? "text-white drop-shadow-sm" : "text-blue-800 dark:text-white drop-shadow-sm";

  return (
    <div aria-label="Support Ukraine banner" className={bannerClasses} ref={bannerRef} role="banner" style={backgroundStyle}>
      <div
        className={`
          container mx-auto flex flex-col items-center justify-between gap-4 px-4 py-4
      ...
      >
        ...
        <div
          ...
          `}
        >
          <Link aria-label="Donate to support Ukraine" className={buttonClasses} href="https://u24.gov.ua" rel="noopener noreferrer" target="_blank">
            Donate
          </Link>
          ...
          {showCloseButton && (
            <button
              ...
              className={`
                banner-content ml-2 rounded-full p-2 opacity-80 transition-opacity duration-200
                focus:ring-opacity-50 focus:ring-2 focus:ring-current focus:outline-none
                hover:opacity-100
                md:ml-4
                ${textColorClasses}
              `}
