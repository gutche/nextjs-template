...
import { NavbarUserDropdown } from "./navbar-user";
      ...
      >
        <div className="flex h-16 items-center justify-between">
            ...
            <Link className="flex items-center gap-2" href="/">
              <span
                className={cn(
                  ...
                  !isDashboard &&
                    `
                      bg-gradient-to-r from-primary to-primary/70 bg-clip-text tracking-tight
                      text-transparent
                    `,
                )}
              >
          ...
          <div className="flex items-center gap-4">
            {!isDashboard && (isPending ? <Skeleton className={`h-9 w-9 rounded-full`} /> : <Cart />)}
            ...
            {showAuth && (
              ...
              >
                {user ? (
                  <NavbarUserDropdown isDashboard={!!isDashboard} userEmail={user.email} userImage={user.image} userName={user.name} />
                ) : isPending ? (
            ...
            {!isDashboard && (isPending ? <Skeleton className={`h-9 w-9 rounded-full`} /> : <ThemeToggle />)}

            {/* Mobile menu button */}
            <Button className="md:hidden" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} size="icon" variant="ghost">
