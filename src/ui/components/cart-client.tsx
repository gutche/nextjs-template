...
import { Drawer, DrawerClose, DrawerContent, DrawerTrigger } from "@/ui/primitives/drawer";
import { Separator } from "@/ui/primitives/separator";
import { Sheet, SheetClose, SheetContent, SheetHeader, SheetTitle, SheetTrigger } from "@/ui/primitives/sheet";
...
export function CartClient({ className, mockCart }: CartProps) {
  ...
  const CartTrigger = (
    <Button aria-label="Open cart" className="relative h-9 w-9 rounded-full" size="icon" variant="outline">
      ...
      {totalItems > 0 && (
        <Badge className={`absolute -top-1 -right-1 h-5 w-5 rounded-full p-0 text-[10px]`} variant="default">
          {totalItems}
        </Badge>
      )}
    </Button>
          ...
          <AnimatePresence>
            {cartItems.length === 0 ? (
              ...
              >
                ...
                <p className="mb-6 text-center text-sm text-muted-foreground">Looks like you haven't added anything to your cart yet.</p>
                {isDesktop ? (
                  <SheetClose asChild>
                    <Link href="/products">
              ...
              <div className="space-y-4 py-4">
                {cartItems.map((item) => (
                  <motion.div
                    ...
                    className={`
                      group relative flex rounded-lg border bg-card p-2 shadow-sm transition-colors
                      hover:bg-accent/50
                    `}
                    exit={{ opacity: 0, y: -10 }}
                    ...
                    <div className="ml-4 flex flex-1 flex-col justify-between">
                      <div>
                        <div className="flex items-start justify-between">
                          ...
                          <button
                            className={`
                              -mt-1 -mr-1 ml-2 rounded-full p-1 text-muted-foreground
                              transition-colors
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <div className="flex items-center rounded-md border">
                          <button
                            className={`
                              flex h-7 w-7 items-center justify-center rounded-l-md border-r
          ...
          <div className="border-t px-6 py-4">
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button className="ml-2" onClick={handleClearCart} variant="outline">
                  Clear Cart
                </Button>
              </div>
            </div>
