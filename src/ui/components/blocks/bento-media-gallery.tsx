...
const GalleryMedia: React.FC<GalleryMediaProps> = React.memo(({ className, isThumbnail = false, item, onClick, onClose, onDelete }) => {
  ...
  if (item.type === "video") {
    return (
      <div
        className={`
          ${className}
          group relative flex items-center justify-center overflow-hidden bg-black/10
        `}
      >
...
const GalleryMediaModal: React.FC<GalleryMediaModalProps> = ({ isOpen, mediaItems, onClose, onDelete, selectedItem, setSelectedItem }) => {
  ...
  const navigateToMediaItem = useCallback(
    (direction: "next" | "prev") => {
      ...
      let newIndex: number;
      if (direction === "next") {
        newIndex = currentIndex === mediaItems.length - 1 ? 0 : currentIndex + 1;
      } else {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      } else if ((event.key === "f" || event.key === "F") && selectedItem.type === "video") {
  ...
  return (
    <>
      <motion.div
        ...
        aria-modal="true"
        className={`
          fixed inset-0 z-10 min-h-screen w-full overflow-hidden rounded-none backdrop-blur-lg
      ...
      >
        <div className="flex h-full flex-col">
          ...
          >
            <motion.button
              ...
              className={`
                absolute top-1/2 left-2 z-20 -translate-y-1/2 rounded-full bg-white/80 p-2
            ...
            <motion.button
              ...
              className={`
                absolute top-1/2 right-2 z-20 -translate-y-1/2 rounded-full bg-white/80 p-2
                text-gray-800 shadow-sm backdrop-blur-sm
                hover:bg-white/90
              `}
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={`
                  relative aspect-[16/9] h-auto max-h-[70vh] w-full max-w-[95%] overflow-hidden
              ...
              >
                ...
                <div
                  className={`
                    absolute right-0 bottom-0 left-0 bg-gradient-to-t from-black/50 to-transparent
                    p-2
                    sm:p-3
                    md:p-4
                  `}
        ...
        <motion.button
          ...
          className={`
            absolute top-2 right-2 z-20 rounded-full bg-white/80 p-2 text-gray-800 shadow-sm
            backdrop-blur-sm
            hover:bg-white/90
      ...
      <div className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2">
        <div
          className={`
            relative rounded-xl border border-blue-400/30 bg-sky-400/20 px-3 py-2 shadow-lg
            backdrop-blur-xl
          `}
        >
          <div className="flex items-center -space-x-2">
            {mediaItems.map((item, index) => (
              <motion.div
                ...
                className={`
                  group relative h-8 w-8 flex-shrink-0 cursor-pointer overflow-hidden rounded-lg
                  hover:z-20
                  sm:h-9 sm:w-9
                  md:h-10 md:w-10
                  ${
              ...
              >
                ...
                <div
                  className={`
                    absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-white/20
                  `}
                />
                {selectedItem.id === item.id && (
                  <motion.div
...
export const BentoMediaGallery: React.FC<BentoMediaGalleryProps> = ({ description, mediaItems, onDelete, title }) => {
    ...
    <div className="container mx-auto max-w-4xl px-4 py-8">
      <div className="mb-8 text-center">
        <motion.h1
          ...
          className={`
            bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 bg-clip-text text-2xl font-bold
            text-transparent
            sm:text-3xl
            md:text-4xl
