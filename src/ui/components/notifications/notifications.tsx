...
import { DropdownMenuGroup, DropdownMenuItem } from "@/ui/primitives/dropdown-menu";
...
export const Notifications: React.FC<NotificationsProps> = ({ notifications, onDismiss, onMarkAsRead }) => {
	...
	return (
		<DropdownMenuGroup className="max-h-[300px] overflow-y-auto">
			{notifications.map((notification) => (
				<DropdownMenuItem
					className={cn("flex cursor-default flex-col items-start p-0", !notification.read && "bg-muted/50")}
					key={notification.id}
					...
					<div className="flex w-full items-start gap-2 p-2">
						...
						<div className="flex flex-shrink-0 gap-1">
							{!notification.read && (
								<Button className="h-6 w-6" onClick={() => onMarkAsRead?.(notification.id)} size="icon" variant="ghost">
									<Check className="h-3 w-3" />
									<span className="sr-only">Mark as read</span>
								</Button>
							)}
							<Button className="h-6 w-6" onClick={() => onDismiss?.(notification.id)} size="icon" variant="ghost">
								<X className="h-3 w-3" />
