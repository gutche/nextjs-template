...
export async function HeroBadge() {
  ...
  return (
    <Link
      className={`
        inline-flex items-center rounded-lg bg-primary/10 px-3 py-1 text-sm font-semibold
        text-primary
      `}
      href={SYSTEM_CONFIG.repoStars ? `https://github.com/${SYSTEM_CONFIG.repoOwner}/${SYSTEM_CONFIG.repoName}` : "/products"}
    ...
    >
      {SYSTEM_CONFIG.repoStars ? (
        <div className="flex items-center gap-1">
          ...
          {githubStars && <span>⭐ {githubStars.toLocaleString()} stars on GitHub</span>}
        </div>
      ) : (
        SEO_CONFIG.fullName
      )}
