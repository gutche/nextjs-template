  ...
  const handleVerify = (e: React.FormEvent) => {
    ...
    try {
      ...
      verifyPromise
        ...
        .catch((err: unknown) => {
          setError(`Invalid ${isUsingBackupCode ? "backup" : "verification"} code. Please try again.`);
          console.error(err);
          setLoading(false);
        })
        .finally(() => {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4">
      <div className="w-full max-w-md space-y-8 rounded-lg border p-6 shadow-md">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">{isUsingBackupCode ? "Enter a backup code" : "Enter your authentication code"}</p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleVerify}>
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                {isUsingBackupCode ? "enter one of your backup codes" : "enter the 6-digit code from your authenticator app"}
              </p>
            </div>
            ...
            {!isUsingBackupCode && (
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  ...
                  <p className="text-gray-500">you won't need to enter a verification code on this device for 30 days</p>
                </div>
              </div>
            )}
            ...
            {isUsingBackupCode && (
              <div className="mt-6">
                ...
                {backupError && <div className={`mb-4 rounded-md bg-red-50 p-4 text-sm text-red-700`}>{backupError}</div>}
                {backupMessage && (
                  <div
                    className={`
                ...
                {backupLoading ? (
                  ...
                  >
                    {backupCodes.map((code) => (
                      <div className={`rounded-md bg-gray-100 p-2 text-center font-mono`} key={code}>
                        {code}
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex justify-center">
                  <button
                    className={`
                      flex items-center justify-center rounded-md bg-blue-600 px-4 py-2 text-sm
          ...
          <div className="flex flex-col space-y-4">
            <button
              className={`
                flex w-full justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium
                text-white
                hover:bg-blue-700
                focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none
                disabled:opacity-50
            ...
            <button
              className={`
                flex w-full justify-center rounded-md bg-gray-200 px-4 py-2 text-sm font-medium
                text-gray-700
                hover:bg-gray-300
