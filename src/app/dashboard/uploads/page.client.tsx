...
export default function UploadsPageClient() {
  const [mediaGalleryItems, setMediaGalleryItems] = useState<GalleryMediaItem[]>([]);
    ...
    <div className="space-y-8">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <LinkIcon
              className={`
                absolute top-1/2 left-2 h-4 w-4 -translate-y-1/2 transform text-muted-foreground
              `}
            />
            <Input
              className="pl-8"
          ...
          <Button disabled={!mediaUrlInput || isUploadingFromUrl} onClick={uploadMediaFromUrl}>
      ...
      <div>
        ...
        {isMediaLoading ? (
          ...
          >
            {[...Array(4)].map((_, i) => (
              <Skeleton className="aspect-square w-full rounded-md" key={`skeleton-${i}`} />
            ))}
          </div>
          ...
          !error && <p className="text-muted-foreground">No media uploaded yet.</p>
        )}
      </div>
    </div>
  );
